...
export const createModelsApi = (client: BaseAPIClient) => ({
  ...
  async toggleModelStatus(profileId: string): Promise<ModelProfile> {
    return client.request<ModelProfile>(`/models/profiles/${profileId}/toggle-status`, { method: "POST" }, API_PREFIX_VENDOR);
  },
  async uploadTrainingPhotos(files: File[], modelEmail?: string): Promise<{ urls: string[] }> {
    ...
    const response = await fetch(`http://localhost:8000/api/v1/models/upload-photos`, {
      method: "POST",
      headers,
      body: formData,
      credentials: "include",
